...
One can also get a `qspray` polynomial with Julia, by 
setting the argument `poly` to `"qspray"` in the `XXXPol` functions in the 
list returned by `Jack_julia`:
...
microbenchmark(
  ...
  Julia_qspray = julia$JackPol(n, lambda, alpha, poly = "qspray"),
  times = 6L
)
```
